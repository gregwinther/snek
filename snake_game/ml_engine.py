...
class DNN_Engine:
    ...
    def __init__(
        self, initial_games=1000, goal_steps=500, lr=2e-2, max_iter=500, engine=Random()
    ):
        self.engine = engine
        self.model_torch()
    ...
    def __call__(self, state, speed):
        ...
        for action in range(-1, 2):
            x = torch.tensor(
                self.add_action_to_observation(observation, action).reshape(-1, 5)
            )
    ...
    def find_vision(self, state):
        ...
        if f.max() < 20 and l.max() < 20 and r.max() < 20:
            vision = [board[f[0], f[1]], board[l[0], l[1]], board[r[0], r[1]]]
        return vision
...
class CNN_Engine:
    ...
    def __init__(self, initial_games=1000, goal_steps=500, lr=2e-2, max_iter=500):
